  ...
  for (let i = 0; i < Object.size(response['measurement']); i++) {
    ...
    if (obj["properties"].status === "-120") {
      x = L.circleMarker([obj["geometry"]["coordinates"][0], obj["geometry"]["coordinates"][1]],
        { radius: 10, color: "Green", weight: 3, fillOpacity: 0.5 }).addTo(mymap);
    }
    else if (obj["properties"].status === "-50") {
      x = L.circleMarker([obj["geometry"]["coordinates"][0], obj["geometry"]["coordinates"][1]], { radius: 10, color: "red", weight: 3, fillOpacity: 0.5 }).addTo(mymap);
    }
    else {
      x = L.circleMarker([obj["geometry"]["coordinates"][0], obj["geometry"]["coordinates"][1]], { radius: 10, color: "yellow", weight: 3, fillOpacity: 0.5 }).addTo(mymap);
    }
